  ...
  type ModalItem = {
    ...
    images: (string | { src: string; fit: 'cover' | 'contain' })[];  // array of image paths/URLs with optional fit property
    img?: string;      // icon for experiences
    id?: string;       // tracking id for projects
    tags?: string[];   // descriptive tags for projects
    isFavorite?: boolean;  // mark as favorite project
    isWIP?: boolean;   // mark as work in progress
  };
          ...
          <motion.div
            ...
            className={`relative z-10 w-full max-w-5xl max-h-[85vh] flex flex-col md:flex-row overflow-hidden shadow-2xl transition-all duration-300 ${
              isMinecraft 
                ...
                : 'bg-[#0d0d0d] border border-emerald-500/30 rounded-lg'
            }`}
            ...
            <div className={`w-full md:w-[45%] shrink-0 flex flex-col items-center justify-center p-4 relative z-10 ${
              ...
              <div className={`relative w-full aspect-[4/3] overflow-hidden bg-black/60 shadow-inner ${
                ...
                {item.images && item.images.length > 0 ? (
                  (() => {
                    const imgData = item.images[slide];
    ...
    const experiences = (
      [
        { 
          ...
          details: 'During this internship I worked in the production department of Magnum Technology Center, there i was placed under the mentorship of Mr.Kumaraguru Kesavan, who is a senior PLC engineer with years of experience. Under him i learned the basics of ladder logic programming and then emulating a siemens PLC using Studio 5000 & Studio 5000 Logix Emulate. After learning the basics i moved onto basic programming making programs like Sequencial Traffic light, Temp controlled FAN, Automatic Tank refilling... and more. More over during the time of training i was also exposed to various safety standards that followed in systems that are deployed in hazardous location like oil fields, these mainly include barriers(like zenner barriers) and specialized standards issued by the government like IEC safety standards.',
          images: [
        ...
        { 
          ...
          desc: 'Contributed to organizing A national-level hackathon - CarbonX, in collaboration with C-DAC.',
          details: 'Was the Track incharge for Vegathon - a 42Hr hackathon, as a part of CarbonX 2026, in which participants got the chance to develop projects using the indigenous Aries boards. During this time, i came to the realization that organizing events is something i really enjoy, it gives me the chance to give back to the community and spread the love of electronics. I was able to coordinate with various sponsors and mentors to ensure a smooth experience for the participants. After interacting with the participants and getting feedbacks from them, which revealed to us the areas in which we require improvement. This hackathon made me realize that there are people out there just like myself who are willing to excuse their sleep, sit throught the night just to make something come to life. Overall it was a great learning experience for me and it also gave me a sense of fulfillment knowing we pulled off something that everyone loved.',
          images: [
            { src: '/pictures/MC_section/vegathon/1.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/vegathon/3.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/vegathon/4.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/vegathon/5.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/vegathon/6.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/vegathon/7.JPG', fit: 'cover' },
          ],
        },
        { 
          title: 'Media Member', 
          ...
          desc: 'Contributed to recording/capturing various college events and coordinating technical media infrastructure for major celebrations.',
          details: 'Captured the moment in 10+ college events including events like Confluence, Bharatham - Art Festival, many IEDC events, GDSC, celebrations... and many more. Along the way i learned so much about the art of storytelling and the importance of capturing the essence of each event, with seniors helping me learn more about handling a camera and the responsiblities that comes with beign part of the RSET Media team.',
          images: [
            { src: '/pictures/MC_section/media/1.jpg', fit: 'cover' },
            { src: '/pictures/MC_section/media/2.jpg', fit: 'contain' },
            { src: '/pictures/MC_section/media/3.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/media/4.JPG', fit: 'contain' },
            { src: '/pictures/MC_section/media/5.jpeg', fit: 'contain' },
          ],
        },
        { 
          ...
          desc: 'Organized many technical and non-technical events many of which were conducted during activity hour events.',
          details: 'During my tenure in the tech team of the Elctronuats Execom (2025-2026) i have contibuted to hosting, mannaging and leading multiple events. from Designing problem statements to sourcing components to managing teams. Electronauts gave me the oppuurtunity to explore the the world of technical coordination. Along with that i had the opportunity to meet and work with many juniors who will be the future of the electronauts club, carrying the flame of love for the world of electronics.',
          images: [
            { src: '/pictures/MC_section/organizing_volunteering/1.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/organizing_volunteering/2.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/organizing_volunteering/3.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/organizing_volunteering/4.JPG', fit: 'contain' },
            { src: '/pictures/MC_section/organizing_volunteering/5.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/organizing_volunteering/6.JPG', fit: 'contain' },
            { src: '/pictures/MC_section/organizing_volunteering/7.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/organizing_volunteering/8.jpg', fit: 'contain' },
            { src: '/pictures/MC_section/organizing_volunteering/9.jpg', fit: 'cover' },
          ],
        },
        { 
          title: 'GIS-Mapathon', 
          ...
          desc: 'Participated and Won a GIS-Mapathon - hosted in collaboration with NeST Digital and RSET.',
          details: 'In january 2026, i participated in my first ever hackathon which was a mapathon hosted by RSET in collaboration with NeST Digital. In it we choose argubly the harder problem statement from the bunch - pavement extraction from satellite imagery. even though we had no idea on how to use the new QGIS software or how to run a AI model in our local GPU of our laptop, we spent the first day of the hackathon learning about everything we need getting a good overiew and understanding the needs of the project. then azfter that we started our training on small data sets which prove the concept, but our limited dataset and limits of the local gpu hindered our progress. we came in 1st for story mapping(which was a sub competition) and top 9 for the main mapathon, this event was a testiment to what we would do if we put our mind to it no matter the challenege.  ',
          images: [
            { src: '/pictures/MC_section/mapathon/1.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/mapathon/2.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/mapathon/3.JPG', fit: 'cover' },
            { src: '/pictures/MC_section/mapathon/4.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/mapathon/5.jpeg', fit: 'cover' },
            { src: '/pictures/MC_section/mapathon/6.jpg', fit: 'cover' },
          ],
        },
      ] as ModalItem[]
    );
  ...
  const SchematicSection = () => {
    ...
    const projects = (
      [
        {
          title: 'Audio Spectrum Analyzer', id: 'ECE_001', tags: ['Analog', 'Filters','DSP','Op-Amps'],
          sub: 'ECE_001 // Analog & DSP',
          highlight: 'border-amber-400',
          isFavorite: true,
          desc: '2 Band Audio Spectrum Analyzer with real-time Audio visualization ',
          details: 'Designed and built a 2 Band Audio Spectrum Analyzer using op-amps, bandpass filters and an LM386 Audio Amplifier. The two bands are bass = 100Hz & treble = 1kHz. Audio input is captured via a 3.5mm jack, filtered through active bandpass filters, amplified to drive the LEDs, The signal then passes through a comparator array made of LM324 ICs. The final design was housed in a custom cardboard box enclosure with a knob for volume control of the speaker.',
          images: [
            { src: '/pictures/Projects/Spectrum_Analyzer/Demo2_1.mp4', fit: 'cover' },
            { src: '/pictures/Projects/Spectrum_Analyzer/final_2.jpeg', fit: 'cover' },
            { src: '/pictures/Projects/Spectrum_Analyzer/demo1_3.mp4', fit: 'cover' },
            { src: '/pictures/Projects/Spectrum_Analyzer/assembly_4.jpeg', fit: 'cover' },
            { src: '/pictures/Projects/Spectrum_Analyzer/testing_5.jpeg', fit: 'cover' },
          ],
        },
        {
          title: '4-Bit Binary Adder', id: 'ECE_002', tags: ['Hardware', 'Transistors','Arthmetic circuits'],
          sub: 'ECE_002 // Discrete Logic',
          desc: '4 Bit binary adder implemented using only discrete transistors to demonstrate arthmetic calculation realization at the transistor level.',
          details: 'Implemented NAND-NAND logic using BC547 NPN transistors on a breadboard. After than we implemented the full 1 Bit bianry adder using only transistors Then proceeded to cascadef four such 1 Bit adder cells to achieve 4-bit addition with carry propagation. Final output was be verified by checking the binary values of the inputs and the outputs. a custom box was made using cardboard, one which exposes the circuitary so that it looks more asthetic and discrete ',
          images: [
            { src: '/pictures/Projects/BinaryAdder/demo.mp4', fit: 'cover' },
            { src: '/pictures/Projects/BinaryAdder/final.jpeg', fit: 'cover' },
          ],
        },
        {
          title: 'Robotic Arm Sim', id: 'ECE_003', tags: ['Simulation','ROS2', 'Gazebo', 'Rviz'],
          sub: 'ECE_003 // Robotics',
          desc: 'Simulation of robotic arm which is controlled by the user using a manual interface.',
          details: 'Simulated a 6-DOF robot arm URDF model and simulated it in RVIZ as part of my Honours course in ROS. During the time ROS2 was pretty new so we faced many issues tryign to understand and develop the code and the file architecture needed to acheive this. so after alot of contemplating we decided to downgrade to ROS2-Iron. On doing so we quickly got the simulation up and running and were able to establish a publisher (with sliders) and subscriber model to control the arm. The project was a great learning experience in understanding the ROS ecosystem and how to work with robotic simulations.',
          images: [
            { src: '/pictures/Projects/RoboticArm/Demo1.mp4', fit: 'cover' },
            { src: '/pictures/Projects/RoboticArm/Demo2.mp4', fit: 'cover' },
          ],
        },
        {
          title: 'Freq Multiplier', id: 'ECE_004', tags: ['CD4046', 'PLL'],
          sub: 'ECE_004 // Analog',
          desc: 'Signal synthesis and multiplier circuit for frequency multiplication utilizing Phase-Locked Loop (PLL) stability.',
          details: 'Used the CD4046 PLL IC with a CD4017 divide-by-N counter in the feedback loop to achieve integer frequency multiplication (×2, ×3, ×4, ×5, ×6, ×7, ×8, ×9). The deivce features two methods for taking in input: either internally generate a signal or take in one externally (a toggle switch can be used to select it). A 555 timer in astable mode generates a square wave output with a frequency of 1.1 kHz. this signal is then passed through PLL and then the cd4017 is used to divide the frequency by N and feed it back to the PLL. The output is taken from the PLL which gives us the multiplied frequency. The circuit was tested using an arduino to verify the output frequencies for different values of N, demonstrating stable frequency multiplication across the range. The final design was assembled on a custom 3d printed case with clear labeling of all the terminals.',
          images: [
            { src: '/pictures/Projects/FrequencyMultiplier/final_1.jpeg', fit: 'cover' },
            { src: '/pictures/Projects/FrequencyMultiplier/test_2.jpeg', fit: 'cover' },
            { src: '/pictures/Projects/FrequencyMultiplier/ckt_3.jpeg', fit: 'contain' },
            { src: '/pictures/Projects/FrequencyMultiplier/case.jpg', fit: 'contain' },
          ],
        },
        {
          title: 'D Flip FLop - Cadence', id: 'ECE_005', tags: ['Cadence', 'Analog', 'VLSI'],
          sub: 'ECE_005 // Analog',
          desc: 'Design and simulation of a D flip-flop using Cadence Virtuoso.',
          details: 'Designed a D flip-flop circuit using CMOS technology in Cadence Virtuoso. The design consists of a master-slave configuration with transmission gates and inverters to achieve the desired flip-flop behavior The experience of performing the simulation helped me to understnad the basic steps required when designing any circuit in cadence , ie the rules that must be followed when deciding dimensions of NMOS and PMOS transistors.',
          images: [
            { src: '/pictures/Projects/DFlipFlop/flipflop_1.png', fit: 'cover' },
            { src: '/pictures/Projects/DFlipFlop/test_2.png', fit: 'cover' },
            { src: '/pictures/Projects/DFlipFlop/nand_3.png', fit: 'cover' },
            { src: '/pictures/Projects/DFlipFlop/not_4.png', fit: 'cover' },
            { src: '/pictures/Projects/DFlipFlop/result_5.png', fit: 'cover' },
          ],
        },
        {
          title: 'BT RC Car Control', id: 'ECE_006', tags: ['Arduino', 'Bluetooth', 'LCD','PowerSystem'],
          sub: 'ECE_006 // Embedded',
          isFavorite: true,
          desc: 'Bluetooth controled RC system with real-time LCD display adding personality into the mix.',
          details: 'Designed around an Arduino Uno and HC-05 Bluetooth module. Motor control via L298N H-bridge. A 16×2 LCD displays two eyes combining a cute personality and direction indication. An android app sends commands over bluetooth and then the uno receives the commands analyzes it and then performes the action that is instructed. After the core logic was done, i implemented the power system for the car through multiple iterations, learning from back EMF, current requirements for motors, servos and LCDs ..etc, to improve the design.',
          images: [
            { src: 'pictures/Projects/BT_car/final.jpg', fit: 'cover' },
            { src: 'pictures/Projects/BT_car/inside1.jpeg', fit: 'cover' },
            { src: 'pictures/Projects/BT_car/inside2.jpeg', fit: 'cover' },
            { src: 'pictures/Projects/BT_car/withoutchassie.jpeg', fit: 'cover' },
          ],
        },
        {
          title: 'RISC Processor', id: 'ECE_007', tags: ['VLSI', 'System Verilog', 'HDL'],
        ...
        {
          title: 'ESP32-Walkman', id: 'ECE_008', tags: ['ESP32', 'Audio', 'Embedded Systems'],
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          ...
          >
            <div className="space-y-4">
              <span className="text-[10px] uppercase tracking-[0.4em] text-emerald-500/40 font-bold block">SECTION_03 // HardWare_ARCH</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((proj, i) => (
              <motion.div
                ...
                className={`group border bg-black/70 backdrop-blur-md p-8 hover:bg-black/80 transition-all relative overflow-hidden cursor-pointer rounded-lg ${
                  (proj as any).isFavorite
                    ? 'border-emerald-400/60 hover:border-emerald-300/80 hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]'
              ...
              >
                ...
                {(proj as any).tags.map((tag: string) => (
                    <span key={tag} className="text-[9px] px-2 py-1 border border-emerald-500/30 text-emerald-400/60 uppercase font-black">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {skillCategories.map((category, i) => (
              <motion.div
                ...
                viewport={{ once: false }}
                transition={{ delay: i * 0.1 }}
                className="bg-black/90 border border-[#00d492]/30 p-8 flex flex-col items-center transition-all hover:border-[#00d492] rounded-lg"
              >
                {/* Icon Circle */}
                <div className="w-24 h-24 rounded-full border border-[#00d492]/40 flex items-center justify-center mb-6 text-emerald-400 hover:border-[#00d492] transition-all">
                  {category.icon}
                </div>

                {/* Title */}
                <h3 className="text-lg font-black text-white uppercase tracking-widest mb-6 text-center">
                  {category.title}
                </h3>

                {/* Skills List */}
                <ul className="w-full flex flex-wrap gap-3 justify-center items-center">
                  {category.skills.map((skill, idx) => (
                    <li 
                      key={idx} 
                      className="border-[1px] border-[#00d492]/60 bg-[#00d492]/5 px-4 py-2 rounded-full text-white text-xs font-mono capitalize tracking-tighter"
                    >
                      {skill}
                    </li>
                  ))}
  ...
  const CertificationsSection = () => {
    const certs = [
      ...
      { title: 'Bharatham - Art Festival',            issuer: 'RSET',               date: '2025-2026',    color: 'border-rose-600'    },
      { title: 'Embedded Sensing, Actuation and Interfacing Systems', issuer: 'NPTEL', date: '2026',      color: 'border-orange-400'    },
      // ────────────────────────────────────────────────────────────────────────
    ];

    // Speed is in px/s — fast by default, slows down on hover.
    const FAST_SPEED = 110;  // default speed (no hover)
    const SLOW_SPEED = 60;   // speed when mouse hovers over the section
    ...
    // Smoothly interpolate speedRef toward target for a gentle transition
    useEffect(() => {
    ...
    useEffect(() => {
      ...
      const LERP = 0.04; // how quickly speed transitions (0–1, lower = smoother)
          ...
          <div ref={beltRef} className="flex gap-6 will-change-transform" style={{ width: 'max-content' }}>
            ...
            {[...certs, ...certs].map((cert, i) => (
              <div
                ...
                className={`w-[280px] shrink-0 p-6 bg-black/70 backdrop-blur-md border ${cert.color} rounded-lg border-opacity-40 hover:border-opacity-100 transition-all group`}
              >
      ...
      <section id="contact" className="py-32 bg-[#111111] border-t border-white/10 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          <motion.div 
            ...
            className="mb-20 text-center"
          >
            <span className="text-[10px] uppercase tracking-[0.5em] text-emerald-500/40 font-bold block mb-4">SECTION_06 // ESTABLISH_COMMS</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <motion.button 
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 }}
              className="p-8 bg-black/70 backdrop-blur-md border border-white/10 rounded-lg flex flex-col items-center text-center hover:border-emerald-500/60 hover:bg-black/80 transition-all group relative cursor-pointer"
            >
            ...
            <motion.a 
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className="p-8 bg-black/70 backdrop-blur-md border border-white/10 rounded-lg flex flex-col items-center text-center hover:border-emerald-500/60 hover:bg-black/80 transition-all group cursor-pointer"
            >
